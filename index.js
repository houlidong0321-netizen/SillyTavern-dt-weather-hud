...
function ensurePanelExists() {
    ...
    const panelHtml = `
        <div id="dt_weather_hud_panel">
            <div class="dt-hud-header" id="dt_weather_hud_drag_handle">
                <span class="dt-hud-title">🕰️ 场景信息</span>
                <span class="dt-hud-collapse" id="dt_weather_hud_collapse_btn" title="折叠/展开">▾</span>
            </div>
            <div class="dt-hud-body" id="dt_weather_hud_body">
                <div class="dt-hud-row">
                    <span class="dt-hud-icon">📅</span>
                    <span class="dt-hud-value dt-hud-empty" id="dt_hud_date">暂无数据</span>
                </div>
                <div class="dt-hud-row">
                    <span class="dt-hud-icon">⏰</span>
                    <span class="dt-hud-value dt-hud-empty" id="dt_hud_time">暂无数据</span>
                </div>
                <div class="dt-hud-row">
                    <span class="dt-hud-icon">🌤️</span>
                    <span class="dt-hud-value dt-hud-empty" id="dt_hud_weather">暂无数据</span>
                </div>
            </div>
        </div>
    `;
...
function makePanelDraggable() {
    ...
    const handle = document.getElementById('dt_weather_hud_drag_handle');
...
function updatePanelDisplay() {
    ...
    const $time = $('#dt_hud_time');
    const $weather = $('#dt_hud_weather');

    if (lastInfo) {
        $date.text(lastInfo.date).removeClass('dt-hud-empty');
        $time.text(lastInfo.time).removeClass('dt-hud-empty');
        $weather.text(lastInfo.weather).removeClass('dt-hud-empty');
    } else {
        $date.text('暂无数据').addClass('dt-hud-empty');
        $time.text('暂无数据').addClass('dt-hud-empty');
        $weather.text('暂无数据').addClass('dt-hud-empty');
    }
}
    ...
    <div class="dt-hud-settings-block">
        <div class="inline-drawer">
            ...
            <div class="inline-drawer-content">
                ...
                <div class="dt-hud-hint">可覆盖 #dt_weather_hud_panel / .dt-hud-header / .dt-hud-body / .dt-hud-row 等选择器。</div>
...
function bindSettingsEvents() {
    ...
    $('#dt_hud_reset_pos_btn').on('click', function () {
        ...
        saveSettings();
        const $panel = $('#dt_weather_hud_panel');
        $panel.css({ left: 'auto', top: '80px', right: '20px' });
    });
